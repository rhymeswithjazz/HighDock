...
A setup matches both its original layout and its chosen main-display layout. On reconnect, a unique setup also matches when only the main display differs. Exact matches take priority, and multiple fallback candidates leave the layout untouched. The app suppresses reapplication caused by its own display change. Conflicting saved setups are rejected before writing. Clearing the choice on a connected setup keeps the current main display and saves that current layout.
