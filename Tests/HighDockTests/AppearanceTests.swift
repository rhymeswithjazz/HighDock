...
func renderAppearanceChecks() async throws {
    ...
    for variant in ["light", "dark", "contrast", "compact", "disabled-light", "disabled-dark", "disabled-contrast"] {
        ...
        let root = MainView(model: model)
            .environment(\.colorScheme, variant.hasSuffix("dark") ? .dark : .light)
        ...
        window.appearance = NSAppearance(named: variant.hasSuffix("dark") ? .darkAqua : (variant.hasSuffix("contrast") ? .accessibilityHighContrastAqua : .aqua))
