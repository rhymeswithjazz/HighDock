...
def release_item(build=8001, beta=False):
    ...
    name = f"HighDock-{build}.zip"
    xml = f'''<rss xmlns:sparkle="{SPARKLE}"><channel><item>
        <sparkle:version>{build}</sparkle:version>{channel}
        <enclosure url="https://github.com/rhymeswithjazz/HighDock/releases/download/personal-{build}/{name}"
...
class PersonalAppcastTests(unittest.TestCase):
    ...
    def test_rejects_upstream_download(self):
        ...
        with self.assertRaisesRegex(ValueError, "HighDock"):
            read_item(xml.replace("rhymeswithjazz/HighDock", "Ranchero-Software/HighDock"), release)

    def test_rejects_missing_zip_and_mismatched_size(self):
        xml, release = release_item()
        for assets in ([], [{"name": "HighDock-8001.zip", "size": 999}]):
