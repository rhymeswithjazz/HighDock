#!/usr/bin/env python3
"""Package a notarized HighDock release, then upload it as a separate draft step."""
...
def setup(args):
    ...
    if existing:
        ...
        if actual != existing:
            raise ValueError("Import HighDock's original private key; do not replace the published key")
    else:
    ...
    print("HighDock's public key matches its private key in Keychain")
...
def preflight(args):
    ...
    if key != public_key() or len(base64.b64decode(key, validate=True)) != 32:
        raise ValueError("HighDock's Keychain signing key does not match Resources/Info.plist")
...
def package(args):
    ...
    app = build_app(output / "export/HighDock.app", version=args.version, build=args.build,
    ...
    expected = {"CFBundleIdentifier": BUNDLE_ID, "CFBundleVersion": str(args.build),
                "CFBundleShortVersionString": args.version, "HighDockUpdatesEnabled": True,
                "SUFeedURL": FEED_URL, "SUPublicEDKey": public_key()}
    if any(info.get(key) != value for key, value in expected.items()):
        raise ValueError("Built app does not match the release settings")
    architectures = run("lipo", "-archs", str(app / "Contents/MacOS/HighDock"), capture=True).split()
    ...
    payload.mkdir()
    zip_path = payload / f"HighDock-{args.build}.zip"
...
def draft(args):
    ...
    expected_names = {f"payload/HighDock-{build}.zip", "appcast.xml", "notes.md"}
    if manifest.get("repository") != REPOSITORY or set(manifest["sha256"]) != expected_names:
        raise ValueError("Release manifest does not match HighDock's artifacts")
...
def main():
    ...
    for name in ("setup", "preflight", "package"):
        ...
        if name == "setup":
            child.add_argument("--create-key", action="store_true", help="Create HighDock's Keychain signing key once and embed its public key")
        if name == "package":
